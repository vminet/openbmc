...
imglist=
whiteimglist="bmc u-boot kernel rofs section section2 u-boot2"
...
if test -n "$doflash"
then
	machine=$(cat /run/initramfs/MANIFEST | grep MachineName | awk '{split($0, a, "="); print a[2]}')
	rm /run/initramfs/MANIFEST
	case $machine in
		"dl360poc") secure_boot=0;;
		"dl325poc") secure_boot=0;;
		"evb-t261") secure_boot=0;;
		*) secure_boot=1;;
	esac
	...
	do
		...
		if [ ! -z "${whiteimglist##*${f#$image}*}" ]; then
			echo "${f#$image} not in the whitelist for flashing"
			continue
		fi
		m=$(findmtd ${f#$image})
		...
		imgsize=$(stat -L -c "%s" "$f")
		curhash=$(head -c $imgsize /dev/$m | sha256sum | awk '{print $1}')
		if [ $curhash = $newhash ]
		then
		...
		then
			...
			then
				...
				then
					...
					dd if=$f bs=1k count=320 of=section.tmp 2> /dev/null
